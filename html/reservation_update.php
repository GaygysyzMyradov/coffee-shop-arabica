...
if(isset($_POST['reservation'])){
    ...
    $qury="update reservation set name='$name',email='$email',seats='$seats',r_date='$ddate' where id='$id'";
    $reg=mysqli_query($connection, $qury);
    if($reg) 
    echo "
    <script>
        window.alert('Reservation Has Been Successfully Updated !');
    ...
    if(isset($_GET['id'])){
        ...
        $rid=$_GET['id'];
        $query="select * from reservation where id='$rid'";
            ...
            <div class="make-reservation-container MRC">
                <div class="heading-h1 primary-font">
                 ...
                 <form action="reservation_update.php" method="POST">
                    ...
                    <input type="hidden" name="rid" value="<?php echo $rows['id']; ?>"  required>
                       
                        <div>
                            <input type="text" placeholder="Name" name="name" value="<?php echo $rows['name']; ?>"  required>
                        </div>
                        <div>
                            <input type="text" placeholder="Email" name="email" value="<?php echo $rows['email']; ?>" required>
                        </div>
                    </div>

                    <div class="form-row-1">
                        <div>
                            <input type="number" placeholder="Number of Seats" value="<?php echo $rows['seats']; ?>" name="seats" required>
                        </div>
                        <div>
                            <input type="date" placeholder="Date" value="<?php echo $rows['r_date']; ?>" name="ddate" required>
                        </div>
                    </div>
                    
                    <div class="input-button">
                        <input type="submit" value="Update" name="reservation">
                    </div>
                </form>
            </div>
